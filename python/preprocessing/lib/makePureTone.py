...
class makePureTone:    
    ...
    def makeTones(self):
        ...
        for i,(f,s) in enumerate(zip(self.freq_pattern,self.spl)):
            ...
            if i < 10:
                w = wave.Wave_write('./0' + str(i) +'_original' + str(f) + '.wav')  # output
            else:
                w = wave.Wave_write('./' + str(i) +'_original' + str(f) + '.wav')  # output
    ...
    def adjustFadein_out(self):
        fileList = glob.glob('./*original*.wav')
        fileList.sort()
        
        for i,f in enumerate(fileList):
        
            sound = AudioSegment.from_file(f, "wav")
            sound1 = sound.fade_in(3).fade_out(3)
            if i < 10:
                sound1.export('./0' + str(i) + '_fadeinout_f' + str(self.freq_pattern[i]) + 
                              'SPL' + str(self.spl[i]) + '.wav', format="wav")
            else:
                sound1.export('./' + str(i) + '_fadeinout_f' + str(self.freq_pattern[i]) + 
                              'SPL' + str(self.spl[i]) + '.wav', format="wav")
