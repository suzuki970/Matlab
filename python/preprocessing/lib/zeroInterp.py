...
def zeroInterp(pupilData,fs,interval,windowL=0):
    ...
    for iTrial in np.arange(pupilData.shape[0]): 
        ...
        if len(zeroInd) > 0:
            ...
            for db in datOfblinkCood:
                data_test.append(pupilData[iTrial,db[0]:db[1]])
                data_control_bef.append(pupilData[iTrial,(db[0]-(db[1]-db[0])-1):db[0]])
                data_control_aft.append(pupilData[iTrial,db[1]:(db[1]+(db[1]-db[0])+1)])
